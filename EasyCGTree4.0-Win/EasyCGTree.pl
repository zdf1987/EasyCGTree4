...
if ($task eq "all" || $task eq "hmmsearch") {
	...
	print LOG "\n############### Task 1: HMM search ###############\n#============= Step 1: HMM search ==============# \n";
	if (-e $TEMdir) {
		my $del1="rd /S /Q $TEMdir";                ##############Different from Linux version
		system ($del1); 
	...
	foreach my $file (@query) {
		...
		$cmd = ".\\bin\\hmmsearch.exe --tblout ./$TEMdir/TEM1_HMMsearch_out/$file -E $evalue --cpu $thread ./HMM/$hmm.hmm ./$inputDir/$file";    ###############Different from Linux version
		
		my $response1= `$cmd` || die "Step 1: Can't execute 'hmmsearch.exe': $!\nPlease make sure: 'hmmsearch.exe' is present in directory 'bin'.\n";  
...
if ($task eq "all" || $task eq "refine") {
	...
	if (-e "$TEMdir\\TEM2_HMMsearch_outS") {###############Different from Linux version
		my $del2="rd /S /Q $TEMdir\\TEM2_HMMsearch_outS"; ###############Different from Linux version
		system ($del2);
...
if ($task eq "all" || $task eq "alignment") {
	...
	if (-e "$TEMdir\\TEM3_GeneSeqs") {####################
		my $del2="rd /S /Q $TEMdir\\TEM3_GeneSeqs";  ################Different from Linux version
		system ($del2);
	...
	print "=====\nIn total, $numg sequence files has been written to '$TEMdir/TEM3_GeneSeqs'!=====\n";
	print LOG "=====\nIn total, $numg sequence files has been written to '$TEMdir/TEM3_GeneSeqs'!=====\n";
...
#============= Step 4: Gather the Orthologs in Single File ==============# 
	...
	if (-e "$TEMdir\\TEM4_GeneCluster") {################Different from Linux version
		my $del2="rd /S /Q $TEMdir\\TEM4_GeneCluster";  ################Different from Linux version
		system ($del2);
...
#============= Step 5: Do Alignment ==============# 
	...
	if (-e "$TEMdir\\TEM5_Alignment") {##Different from Linux version
		my $del2="rd /S /Q $TEMdir\\TEM5_Alignment";  ##Different from Linux version
		system ($del2);
	...
	foreach (@fgene) {
		...
		my @para = ("-in", "./$TEMdir/TEM4_GeneCluster/$_.fas", "-out", "./$TEMdir/TEM5_Alignment/$outname");##Different from Linux version

		unless (system "$program", @para) {##Different from Linux version
			$num7++;
			print "\nStep 5: $_ has been completed! It is the $num7/$fgn file.\n\n";
		} else {
			die "Step 5: Can't execute '.\\bin\\muscle.exe': $!.\n";   ##Different from Linux version
		}
	}
	print LOG "\nIn total, $num7 alignments has been created in '$TEMdir/TEM5_Alignment'!\n\n";

#============= Step 6: Trim Algnments ==============# 
	print "\n\n\n#============= Step 6: Trim Algnments ==============#\n\n";
	print LOG "\n\n\n#============= Step 6: Trim Algnments ==============#\n\n";
	
	if (-e "$TEMdir\\TEM6_AlnTrimmed") {##Different from Linux version
		my $del2="rd /S /Q $TEMdir\\TEM6_AlnTrimmed";  ##Different from Linux version
		system ($del2);
	...
	foreach my $filex (@fgene) {
		...
		my $cmd76 = ".\\bin\\trimal.exe -in $TEMdir/TEM5_Alignment/$file -out $TEMdir/TEM6_AlnTrimmed/$outname -$trim";##Different from Linux version
		system ($cmd76);
		$num8++;
	...
	print LOG "\nIn total, $num8 alignments has been trimmed successfully and written to '$TEMdir/TEM6_AlnTrimmed'!\n\n";
	print "\n\nIn total, $num8 alignments has been trimmed successfully and written to '$TEMdir/TEM6_AlnTrimmed'!\n\n";
...
#============= Step 8: Phylogeny Inference ==============# 
		...
		if ($tree_app eq "fasttree") {
			$cmd11=".\\bin\\$fast $inputDir.concatenation.fas > $inputDir.supermatrix.$tree_app.tree";	##Different from Linux version
			...
			$cmd11=".\\bin\\$iq -s $inputDir.concatenation.fas -T $thread";
			system ($cmd11); 
	...
	#============= Step 7: Construct gene Trees ==============#
		...
		print "\n\n############### Task 4: Tree Inference #################\n#============= Step 7: Construct gene Trees ==============#\n\n";
		print LOG "\n\n############### Task 4: Tree Inference #################\n#============= Step 7: Construct gene Trees ==============#\n\n";
		if (-e "$TEMdir\\TEM7_GeneTrees") {##Different from Linux version
			my $del2="rd /S /Q $TEMdir\\TEM7_GeneTrees";  ##Different from Linux version
			system ($del2);
		...
		foreach my $file11 ( @in11) {
			...
			my $cmd11;
			if ($tree_app eq "fasttree") {
				$cmd11=".\\bin\\$fast $TEMdir/TEM6_AlnTrimmed/$file11 > $TEMdir/TEM7_GeneTrees/$file11.tree";	##Different from Linux version
				&FastreeResponse ($cmd11);
			} elsif ($tree_app eq "iqtree") {
				$cmd11=".\\bin\\$iq -s $TEMdir/TEM6_AlnTrimmed/$file11 -T $thread";
				system ($cmd11); 				
				
				##Different from Linux version
				my $mv1="move $TEMdir\\TEM6_AlnTrimmed\\$file11.ckp.gz $TEMdir\\TEM7_GeneTrees\\$file11.ckp.gz";####
				system ($mv1);
				my $mv2="move $TEMdir\\TEM6_AlnTrimmed\\$file11.model.gz $TEMdir\\TEM7_GeneTrees\\$file11.model.gz";####
				system ($mv2);
				my $mv3="move $TEMdir\\TEM6_AlnTrimmed\\$file11.bionj $TEMdir\\TEM7_GeneTrees\\$file11.bionj";####
				system ($mv3);
				my $mv4="move $TEMdir\\TEM6_AlnTrimmed\\$file11.contree $TEMdir\\TEM7_GeneTrees\\$file11.contree";####
				system ($mv4);
				my $mv5="move $TEMdir\\TEM6_AlnTrimmed\\$file11.iqtree $TEMdir\\TEM7_GeneTrees\\$file11.iqtree";####
				system ($mv5);
				my $mv6="move $TEMdir\\TEM6_AlnTrimmed\\$file11.mldist $TEMdir\\TEM7_GeneTrees\\$file11.mldist";####
				system ($mv6);
				my $mv7="move $TEMdir\\TEM6_AlnTrimmed\\$file11.splits.nex $TEMdir\\TEM7_GeneTrees\\$file11.splits.nex";####
				system ($mv7);
				my $mv8="move $TEMdir\\TEM6_AlnTrimmed\\$file11.treefile $TEMdir\\TEM7_GeneTrees\\$file11.iqtreefile";####
				system ($mv8);
				my $mv9="move $TEMdir\\TEM6_AlnTrimmed\\$file11.log $TEMdir\\TEM7_GeneTrees\\$file11.log";####
				system ($mv9);
				my $mv10="move $TEMdir\\TEM6_AlnTrimmed\\$file11.uniqueseq.phy $TEMdir\\TEM7_GeneTrees\\$file11.uniqueseq.phy";####
		...
		print LOG "In total, $num11 phylogenies has been written to '$TEMdir/TEM7_GeneTrees'.\n\n";
		print  "In total, $num11 phylogenies has been written to '$TEMdir/TEM7_GeneTrees'.\n\n";
...
sub FastreeResponse {
	...
	foreach my $res1 (@res11) {
		if  ($res1 =~ /truncated/i && $res1 =~ /be too long/i) {
			print LOG "Task 'tree_infer' ERROR: EasyCGTree stopped when execute 'FastTree':\n$response11\n\n\n\nPlease check whether the genome/proteome files were formated correctly. \n";##Different from Linux version
			die  "Task 'tree_infer' ERROR: EasyCGTree stopped when execute 'FastTree':\n$response11\n\n\n\nPlease check whether the genome/proteome files were formated correctly. \n";##Different from Linux version
